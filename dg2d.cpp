...
public:
	...
	create_mesh(mesh_type& msh, size_t refinement_iterations)
	{
		...
		std::cout << msh.points.size() << std::endl;
	...
	refine_mesh(mesh_type& msh, size_t refinement_iterations)
	{
		...
		{
			triangles.clear();
			std::copy(msh.cells.begin(), msh.cells.end(), std::back_inserter(triangles) );

			refine_mesh(msh);

			msh.cells.clear();
			msh.cells.resize( triangles.size() );
			std::copy(triangles.begin(), triangles.end(), msh.cells.begin());
		}
	...
	{
		std::cout << "Mesh refinemnt" << std::endl;
		std::cout << msh.points.size() << std::endl;
		std::cout << msh.edges.size() << std::endl;
		std::cout << msh.cells.size() << std::endl;
		 /* break all the edges */
...
int main(int argc, char **argv)
{
	...
	simplicial_mesh<T> mesh;
	auto mesher = get_mesher(mesh);
